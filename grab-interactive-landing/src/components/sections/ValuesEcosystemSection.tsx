...
export function ValuesEcosystemSection() {
  ...
  return (
    <section
      ref={sectionRef}
      id="nilai-keamanan"
      className="relative z-10 h-screen min-h-screen w-full overflow-hidden bg-[#F8FAF9] dark:bg-[#07160D] py-14 text-[#0A1A12] dark:text-white flex flex-col justify-center select-none transition-colors duration-300"
    >
      ...
      <div className="relative mx-auto w-full max-w-[1280px] px-6 md:px-10">
        ...
        <div className="values-header-box text-center">
          <h2 className="values-heading text-3xl font-extrabold leading-tight tracking-tight text-[#0A1A12] dark:text-white md:text-5xl">
            Inovasi Berdampak Nyata bagi<br />
            <span className="text-primary">Masyarakat &amp; Masa Depan</span>
          </h2>
          <p className="values-desc mx-auto mt-3 max-w-2xl text-sm leading-relaxed text-[#5B6B62] dark:text-white/70 md:text-base">
        ...
        <div className="values-card-grid mt-10 md:mt-14 grid gap-6 md:grid-cols-3 md:gap-7">
          {VALUES_DATA.map((item) => {
            const Icon = item.icon;
            return (
              <div
                key={item.id}
                className="value-card-box group relative flex flex-col justify-between overflow-hidden rounded-[28px] border border-black/8 dark:border-white/10 bg-white dark:bg-[#0E2317]/80 p-7 shadow-[0_16px_40px_rgba(0,0,0,0.06)] dark:shadow-[0_16px_40px_rgba(0,0,0,0.3)] backdrop-blur-xl transition-all duration-300 hover:-translate-y-2 hover:border-primary/40 hover:shadow-[0_24px_50px_rgba(0,177,79,0.2)] md:p-8"
              >
                {/* Glow Spot */}
                ...
                <div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex h-13 w-13 items-center justify-center rounded-2xl bg-gradient-to-br from-primary to-emerald-600 text-white shadow-md shadow-primary/25 transition-transform duration-300 group-hover:scale-110">
                      <Icon className="h-6 w-6" />
                    </div>
                    <span className="font-mono-tag text-[10px] font-black tracking-widest text-primary bg-primary/10 px-3 py-1 rounded-full border border-primary/20">
                      {item.tag.toUpperCase()}
                    </span>
                  </div>

                  {/* Title & Desc */}
                  <h3 className="mt-5 text-xl font-black text-[#0A1A12] dark:text-white leading-snug">
                    {item.title}
                  </h3>
                  <p className="mt-3 text-sm leading-relaxed text-[#5B6B62] dark:text-white/75">
                    {item.desc}
                  </p>

                  {/* Highlights Checklist */}
                  <div className="mt-6 space-y-2.5 border-t border-black/5 dark:border-white/10 pt-5">
                    {item.highlights.map((point) => (
                      <div key={point} className="flex items-start gap-2.5 text-xs font-semibold text-[#0A1A12] dark:text-white/90">
