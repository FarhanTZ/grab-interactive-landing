  ...
  return (
    ...
    >
      ...
      <div className="relative mx-auto flex w-full max-w-[900px] flex-col items-center justify-center text-center z-20">
        ...
        {/* Scroll Indicator Prompt (Smooth Floating, Tidak Geter-Geter) */}
        <div className="transition-prompt mt-12 flex flex-col items-center gap-3">
          <span className="text-xs sm:text-sm font-black uppercase tracking-[0.3em] text-primary/90">
            Scroll untuk Menjelajah
          </span>
          <button
            type="button"
            onClick={() => {
              const el = document.getElementById('histori-perjalanan');
              el?.scrollIntoView({ behavior: 'smooth' });
            }}
            title="Mulai Jelajahi Histori"
            aria-label="Mulai Jelajahi Histori"
            className="group flex h-12 w-12 items-center justify-center rounded-full bg-white dark:bg-[#0F1D14] border-2 border-primary/40 shadow-lg text-primary transition-all duration-300 hover:scale-110 hover:bg-primary hover:text-white cursor-pointer active:scale-95 will-change-transform animate-[bounce_2s_infinite]"
          >
